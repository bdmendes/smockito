...
import Mock.mapper.*
import com.bdmendes.smockito.SmockitoException.*
...
extension [T](mock: Mock[T])
          ...
          stubArgClasses
            ...
            .forall: (s, m) =>
              m.isAssignableFrom(s) ||
                (s.isPrimitive && m == classOf[Object]) || classOf[Function0[?]].isAssignableFrom(m)
      .toList
  ...
  /** Sets up a stub that delegates to the real implementation of this method. Useful when you want
    * to preserve an adapter method’s behavior while stubbing a method lower in the hierarchy later
    * with [[on]]. Otherwise, use with care.
    *
    * Notice that, if the real implementation interacts with a class value that is not available in
    * the mocking context, the stub will throw with a [[java.lang.NullPointerException]]. Similarly,
    * if this method is abstract on the mocked type, this will throw at set up time.
    *
    * @param method
  ...
  /** Sets up a stub for a method that behaves differently based on the call number. For the version
    * operating only on the expected set of inputs, see [[on]].
    *
    * @param method
    *   the method to mock.
    * @param stub
